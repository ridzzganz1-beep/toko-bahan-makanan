...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-400 mt-2">Masukkan informasi barang yang ingin Anda tambahkan ke dalam sistem</p>
    </div>

    <!-- Form Card -->
    <div class="rounded-2xl glass border border-white/10 p-8 shadow-xl">
        <form id="barangForm" action="{{ route('admin.barangs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div>
                ...
                <p class="text-slate-500 text-xs mt-2">Format: JPG, PNG, GIF, SVG. Maksimal 2MB</p>
                @error('gambar')
                    <p class="text-rose-400 text-sm mt-2 flex items-center gap-1">
                        <i class="fas fa-exclamation-circle"></i>
                        {{ $message }}
            ...
            <div>
                ...
                <ul class="space-y-2 text-slate-400 text-sm">
                    <li class="flex items-center gap-2">
                        ...
                        Masukkan nama barang yang jelas dan deskriptif
                    </li>
                    <li class="flex items-center gap-2">
                        <span class="w-1.5 h-1.5 bg-sky-400 rounded-full"></span>
                    ...
                    <li class="flex items-center gap-2">
                        <span class="w-1.5 h-1.5 bg-sky-400 rounded-full"></span>
                        Anda dapat mengedit barang kapan saja setelah disimpan
                    </li>
                </ul>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (errorMessages.length > 0) {
        ...
        if (submitBtn) {
            ...
            submitBtn.innerHTML = submitBtn.dataset.originalText || '<i class="fas fa-save"></i> Simpan Barang';
        }
    }
});
</script>
